    ...
    if step == 1:
        ...
        for epoch in range(opt.start_epoch, opt.epochs + 1):
            ...
            if epoch % opt.save_every == 0:
                save_every_checkpoint(model, optimizer, epoch, step, scale)
    ...
    for iteration, batch in enumerate(test_data_loader):
        ...
        with torch.no_grad():
            ...
            if not os.path.exists('result'):
                os.makedirs("result/")
            if opt.test_only:
                if not os.path.exists('result/{}'.format(opt.video_test)):
                    os.makedirs('result/{}'.format(opt.video_test))
                utils.save_image(sr_, 'result/{}/{}.png'.format(opt.video_test, filename[0]))
            else:
...
def save_every_checkpoint(model, optimizer, epoch, step=1, scale=2):
    model_out_path =  "checkpoint/model_s{}_{}x_{}.pth".format(step, scale, epoch)
    state = {"epoch": epoch ,"model": model, 'optimizer': [optimizer[0].state_dict(), optimizer[1].state_dict()]}
